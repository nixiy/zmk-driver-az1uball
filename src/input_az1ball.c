...
static int az1ball_write_count_type(const struct device *dev) {
    const struct az1ball_config *cfg = dev->config;
    uint8_t cmd = (cfg->count_type == 1) ? 0x91 : 0x90;
...
static int az1ball_init(const struct device *dev) {
    ...
    int ret = az1ball_write_count_type(dev);
    if (ret < 0) {
        LOG_WRN("Failed to set count type: %d", ret);
    }
